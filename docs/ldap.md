...
- dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp (ルート)
  ...
  - o=Systems (システム関連)
    - ou=Group (GID)
  - sambaDomainName=FS (samba サーバ情報)

| Type  | Description                                       |
|:------|:--------------------------------------------------|
|FELLOW | 共同研究者，半永続的 (= 利用期限がない) 場合にはここに登録 |
|GUEST  | ゲストアカウント，一時的な利用の場合にはここに登録        |
|HPCS   | HPCS 研究室の教員・学生はここに登録                    |

## LDAPセットアップ

インストール等はOSによって異なるのでよしなに。
以下のldifファイルを適用する。コマンドは
...
olcAccess: {0}to * by dn.base="gidNumber=0+uidNumber=0,cn=peercred,cn=external,cn=auth"
  read by dn.base="cn=Manager,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp" read by * none

dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcSuffix
olcSuffix: dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp

dn: olcDatabase={2}hdb,cn=config
changetype: modify
replace: olcRootDN
olcRootDN: cn=Manager,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
...
```sh
ldapmodify -x -D cn=config -W -f ./change_domain.ldif
```

### データ構造定義

```ldif
dn: dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: dcObject
objectClass: organization
objectClass: top
dc: hpcs
o: HPCS Laboratory
description: HPCS Laboratory, University of Tsukuba Japan

dn: cn=Manager,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: organizationalRole
objectClass: top
cn: Manager
description: Directory Manager

dn: o=HPCS,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
o: HPCS
objectClass: organization
objectClass: top
description: High-Performance Computing System Lab.

dn: ou=Group,o=HPCS,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: organizationalUnit
objectClass: top
ou: Group

dn: ou=People,o=HPCS,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: organizationalUnit
objectClass: top
ou: People

dn: o=FELLOW,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
o: FELLOW
objectClass: organization
objectClass: top
description: Fellow Account

dn: ou=People,o=FELLOW,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: organizationalUnit
objectClass: top
ou: People

dn: o=GUEST,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
o: GUEST
objectClass: organization
objectClass: top
description: Guest Account (not Fellow)

dn: ou=People,o=GUEST,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp
objectClass: organizationalUnit
objectClass: top
ou: People
```
...
```sh
ldapadd -x -D "cn=Manager,dc=hpcs,dc=cs,dc=tsukuba,dc=ac,dc=jp" -W -f ./base.ldif
```
...
```sh
ldapadd -x -W -D cn=config -f ./samba.ldif
```

### バックアップ

`/local/rescue/`にバックアップファイルがあるので`slapadd -l <ファイル>`でロードする。
バックアップを作るときは`slapcat -l <ファイル>`で出力
